...
class Faculty
{
    ...
    function faculty_report($subj_alloc_id){
        ...
        function star_wise_report($faculty_no){
            ...
            $output.="<th class='text-capitalize text-dark text-center info'> NUMBER OF<br/>FEEDBACKS </th>";
        ...
        $f_=mysqli_query($this->conn,"select id from faculties where employee_number='$faculty_no' ");
            ...
            $sa_=mysqli_query($this->conn,"select id,class_id from subject_allocations where faculty_id=$faculty_id[0] and 
                 class_id in (select id from classes where isActive=1) ");
            while($subj_alloc_id=mysqli_fetch_assoc($sa_)){
                $class_strength_=mysqli_query($this->conn,"select count(id) from students where class_id=".$subj_alloc_id['class_id']." ");
                $class_strength=mysqli_fetch_array($class_strength_);
                $class_tot+=$class_strength[0];
                $count_phase_1_=mysqli_query($this->conn,"select count(sum) from feedbacks where sa_id=".$subj_alloc_id['id']." and phase=1 ");
                $count_phase_1=mysqli_fetch_array($count_phase_1_);
                $count_phase_2_=mysqli_query($this->conn,"select count(sum) from feedbacks where sa_id=".$subj_alloc_id['id']." and phase=2 ");
                $count_phase_2=mysqli_fetch_array($count_phase_2_);
                $count_phase1+=$count_phase_1[0];
                $count_phase2+=$count_phase_2[0];
            ...
            for($j=0;$j<10;$j++){
                ...
                //$output.="<td>".$count_phase1." / ".$class_tot."</td>";
                //$output.="<td>".$count_phase2." / ".$class_tot."</td>";
                $output.="<td class='text-center'>".($count_phase1+$count_phase2)."</td>";
                ///////////////////////
                for($k=5;$k>0;$k--){
