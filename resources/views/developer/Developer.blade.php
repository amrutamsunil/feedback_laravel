...
public function __construct($conn)
{
    ...
    /// modify the code to delete the feedback record of only the current semester
    function DeleteFeedbackReport($student_reg){
        ...
        if(mysqli_query($this->conn,$query)){
            $output="<h1>SUCCESFULLY DELETED ID:$student_id[0]</h1>";}
        else {
    ...
    function update_password($op,$np,$cp,$user){
        ...
        if($op!=$pass)
        {
            ...
            $err_="You Entered worng old passsword!!";
        }
    ...
    function add_new_subject($subject_name,$short_name){
        $c=mysqli_query($this->conn,"select id from subjects where name='$subject_name' and short='$short_name' ");
        ...
        $query=mysqli_query($this->conn,"insert into subjects(name,short) values('$subject_name','$short_name')");
        if(!$query)
        {$status="<h4 style='color: red'>ERROR IN ADDING !!</h4>"; return $status;}else {
            $status = "<h4 style='color:green'>SUCCESSFULLY ADDED NEW Subject $subject_name</h4>";
        }
        return $status;
    }
