...
namespace admin_ns;
    class Admin_Class
    {
        ...
        function LoginCheck($username,$password,$err){
            ...
            {
                    $login = mysqli_query($this->conn, "select * from admin where name='$username' and password='$password'");
                    $login_details = mysqli_num_rows($login);
                    $d_temp=mysqli_fetch_array($login);
                if($login_details==true)
                {
                    $_SESSION['user']=$username;
                    $_SESSION['dept_id']=$d_temp[3];
        ...
        function Class_lists2($dept_id,$prev_class_id,$prev_class_name)
        {
                if($prev_class_id!=""){
                $options = "<option value='".$prev_class_id."' selected>".$prev_class_name."</option> ";
                }else{
                $options = "<option value=''>Select Class</option> ";}
   ...
   function Student_status($class_id){
       ...
       return $tbl;
        ...
        {
            $options = "<option value=''>Select Faculty</option>";
            $query = "select id,name,employee_number from faculties where 1=1";
            $fn = mysqli_query($this->conn, $query);
            if (!$fn) {
                $options .= "<option>error</option>";
            }
            while ($faculty = mysqli_fetch_array($fn)) {
                $options .= "<option value='" . $faculty['id'] . "'>" . $faculty['name'] . "(".$faculty['employee_number'].") </option>";
            }
            return $options;
        }
        ...
        function  show_time_table($class_id){
            $output="";$index=0;
            $a = mysqli_query($this->conn, "select * from subject_allocations where class_id=$class_id");
            while ($b = mysqli_fetch_array($a)) {
                $subj_id = $b['subject_id'];
                ...
                $output .= "<td >" . $subject_name['name'] . "</td>";
                $output .= "<td>" . $faculty_name['name'] . "</td>";
                $output.="<td><input type='button' value='Delete' class='btn btn-danger' onclick='delete_subj_alloc($b[0])'></td>";
                $output .= "</tr>";
            }
            return $output;
